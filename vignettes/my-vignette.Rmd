...
n_samples <- 10
set.seed(12)
folds <- createFolds(training$Class, n_samples)
...
We now decide on a set of caret methods we wish to train on our training data. Then we launch runs on domino by programmatically passing our caret methods as arguments to the script `variable_model.R`
...
launcher(script = "variable_model.R", dataset=dataset, par=models_breast)
```
Notice that we pass the name of the dataset as a variable. This is so that we tag our .Rda files with a name we can use to load the files in the future. We could use a more sophisticated name convention for the dataset, or any name want for that matter. The only key is to keep track of the name we use so that we are able to load the results of our model training in the next step.
...
blender_fact <-  function(ensemble){
  ...
  df <- data.frame(Cl.thickness = Cl.thickness, Cell.size = Cell.size, Cell.shape = Cell.shape, Marg.adhesion = Marg.adhesion, Epith.c.size = Epith.c.size, 
                    Bare.nuclei = Bar.nuclei, Bl.cromatin =Bl.cromatin, Normal.nucleoli =Normal.nucleoli, Mitoses = Mitoses)
...
```
curl -v -X POST \
    https://endpoint.dominodatalab.com/v1/benjaminrollert/UseRgroup/endpoint \
    -H "Content-Type: application/json" \
    -H 'X-Domino-Api-Key: '$DOMINO_API'' \
